...
- `RuntimeCoordinator` 按 Tower session 隔离 DriverSession，维护单 active turn、turnId/sequence、权限状态、迟到事件过滤和可等待销毁。同一 Tower session 的 disposal promise 与可取消 start admission lease 共同构成 runtime admission gate：SessionManager 必须持有 lease 完成 terminal/generation 推进、credential 签发、durable launch claim 和 `runTurn` 的 child/DriverSession 启动交接，但不得持有到长 turn 完成。公共 initial start/follow-up 在进入 runtime lease 前还必须登记可取消的 session operation；stop intent 要同步撤销已登记和停止期间新登记的 operation，避免 DB、auto-commit 或 source 查询窗口在 disposal 完成后又取得 lease。所有 background cleanup、显式 stop、retry 和 shutdown disposal 都要先同步发布 disposal intent 并取消当前 lease，再有界等待其把 credential/claim 收尾；lease 释放前不得 close/delete DriverSession，disposal 存在时不得取得新 lease。等待失败时保留 managed owner/evidence 并返回可重试错误，不能并发打开 replacement runtime。
- CLI Driver 选择 Executor，并拥有 PTY、AgentPipeline、Parser、early event 和真实 child 退出跟踪。
- 通用 ACP Driver 拥有 adapter/native ACP process、initialize、session new/load、prompt/cancel、权限响应和协议清理；ACP stdout 不进入普通日志。每次 transport reset 都必须推进 generation；`connect` 在 reset、launch resolve、spawn/started evidence、initialize/authenticate 和 session bootstrap 等异步边界后复核 closed、admission signal、generation 与 connection identity，`runTurn` 即使复用 ready session 也必须在最后一个 await 后再复核同一 transport 才可发送 prompt。每代 ACP client notification/permission handler 必须绑定建立它的 generation 与 connection，旧 generation 的迟到结果只能清理其本地 ProcessManager/launch owner，不能 spawn、覆盖新 transport、写入新 projector/bootstrap 或创建权限请求。
