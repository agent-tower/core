...
Agent 在 Team Room 消息中发送 localhost 链接时，点击链接会打开该消息来源 workspace 的 Preview，而不是访问当前手机或电脑自己的 localhost。

## 成员策略

成员配置会在创建 TeamRun 时快照保存，后续修改预设不会自动改写已存在的 TeamRun 成员。
...
TeamRun 支持公开 RoomMessage 和私聊消息。私聊消息只对发送者和收件成员可见；MCP 工具会按当前 TeamRun 成员身份过滤可见消息。

房间消息列表可能只返回预览内容。需要完整正文时，使用消息详情接口或 MCP 的 `get_room_message`。
