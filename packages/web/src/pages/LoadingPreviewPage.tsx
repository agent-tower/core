    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border px-6 py-4">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {t('10 个首屏加载动画候选 · 颜色遵循 Agent Tower 设计规范 · 点击卡片查看全屏效果')}
          </p>
        </div>
      </header>
      ...
      >
          ...
          {LOADING_ANIMATIONS.map((anim, idx) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">{t(anim.description)}</p>
              </button>
            )
          })}
        </div>
        ...
        <div className="mt-10 p-5 border border-border rounded-lg bg-muted/30">
          <h2 className="text-sm font-semibold text-foreground mb-2">{t('设计推荐')}</h2>
          <ul className="text-xs text-muted-foreground space-y-1.5 leading-relaxed">
            <li>
              <strong className="text-foreground">{t('首选：Charcoal Breathe (#10)')}</strong> — {t('品牌感最强，Charcoal 色块+Tower 图标呼吸，用户对产品的第一印象即品牌本身。')}
            </li>
            <li>
              <strong className="text-foreground">{t('备选A：Tower Pulse (#1)')}</strong> — {t('品牌图标直接出现，脉冲节奏稳定，不浮夸，适合工作台场景。')}
            </li>
            <li>
              <strong className="text-foreground">{t('备选B：Terminal Cursor (#4)')}</strong> — {t('终端光标闪烁，贴合 Agent/终端协作的核心场景，开发者亲切感强。')}
            </li>
          </ul>
        </div>
      </main>
      ...
      {focused && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={t('全屏预览：{name}', { name: focused.name })}
        >
          <button
            ref={closeBtnRef}
            onClick={closeDialog}
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="text-center mt-4">
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-sm">{t(focused.description)}</p>
            </div>
            <span className="text-xs text-muted-foreground/60 mt-2">{t('按 Esc 或点击关闭按钮退出')}</span>
          </div>
        </div>
      )}
    </div>
