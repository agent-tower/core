    ...
    <div role="alert" className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-red-600">{getErrorMessage(error, t('请稍后重试。'))}</div>
        </div>
  ...
  return (
    <SettingsPageContainer className="max-w-6xl">
      <div className="flex items-center justify-between gap-4 mb-5">
        <h2 className="text-base font-semibold text-neutral-900">{t('团队协作设置')}</h2>
        <div className="inline-flex rounded-lg border border-neutral-200 bg-neutral-50/80 p-0.5">
          <button type="button" onClick={() => { setActiveTab('presets'); backToList() }} className={cn('rounded-md px-3 py-1.5 text-[12px] font-medium transition-all', activeTab === 'presets' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-900')}>
            {t('成员预设')}
          </button>
          <button type="button" onClick={() => { setActiveTab('templates'); backToList() }} className={cn('rounded-md px-3 py-1.5 text-[12px] font-medium transition-all', activeTab === 'templates' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-900')}>
            {t('团队模板')}
          </button>
        </div>
      </div>
      ...
      {activeTab === 'presets' ? (
        mobileShowEditor ? (
          ...
          <div className="rounded-xl border border-neutral-200 bg-white">
            <div className="flex items-center justify-between gap-3 border-b border-neutral-100 px-5 py-3">
              <div className="flex items-center gap-3">
                <button onClick={backToList} className="flex items-center gap-1.5 text-xs text-neutral-500 hover:text-neutral-900 transition-colors">
                  <ArrowLeft size={14} />
                  {t('返回')}
                </button>
              ...
              <div className="flex items-center gap-2">
                {presetMode === 'edit' && selectedPreset && (
                  <button onClick={() => setDeletePresetTarget(selectedPreset)} className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-400 hover:text-red-500 hover:bg-red-50 transition-colors" title={t('删除')}>
                    <Trash2 size={14} />
                  </button>
                )}
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-start gap-4">
                <div className="shrink-0">
                  ...
                  <div className="flex justify-center gap-1 mt-2">
                    <button type="button" onClick={() => setIsAvatarPickerOpen(true)} className="text-[10px] text-neutral-400 hover:text-neutral-700 transition-colors">{t('选择')}</button>
                    <span className="text-[10px] text-neutral-300">·</span>
                    <button type="button" onClick={() => avatarFileInputRef.current?.click()} disabled={isUploadingAvatar} className="text-[10px] text-neutral-400 hover:text-neutral-700 transition-colors">
                      {isUploadingAvatar ? t('...') : t('上传')}
                    </button>
                    {presetForm.avatar.trim() && (
                      <>
                        <span className="text-[10px] text-neutral-300">·</span>
                        <button type="button" onClick={handleClearAvatar} className="text-[10px] text-neutral-400 hover:text-red-500 transition-colors">{t('清除')}</button>
                      </>
                    )}
                  </div>
                ...
                <div className="flex-1 min-w-0 grid gap-3 sm:grid-cols-2">
                  <div>
                    <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('名称')}</label>
                    <input value={presetForm.name} onChange={e => updatePresetField('name', e.target.value)} className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm transition-colors focus:border-neutral-400 focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('别名')}</label>
                    <input value={presetForm.aliasesText} onChange={e => updatePresetField('aliasesText', e.target.value)} placeholder={t('alice, a, reviewer')} className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm transition-colors focus:border-neutral-400 focus:outline-none" />
                  </div>
                </div>
              </div>

              <div>
                <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('Provider')}</label>
                ...
                {providersIsError && <p className="mt-1 text-[11px] text-red-500">{t('Provider 列表加载失败。')}</p>}
              </div>

              <div>
                <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('角色提示词')}</label>
                <textarea value={presetForm.rolePrompt} onChange={e => updatePresetField('rolePrompt', e.target.value)} rows={4} className="w-full rounded-lg border border-neutral-200 bg-neutral-50/50 px-3 py-2 text-sm font-mono transition-colors focus:border-neutral-400 focus:bg-white focus:outline-none resize-y" />
              </div>

              <div>
                <div className="flex items-end justify-between gap-3 mb-2">
                  <label className="text-[12px] font-medium text-neutral-600">{t('可用能力')}</label>
                  <span className="text-[11px] text-neutral-400">{selectedPresetCount}/10</span>
                </div>
                <div className="grid gap-1 sm:grid-cols-2 xl:grid-cols-3">
                  {CAPABILITY_FIELDS.map(field => (
                    <label key={field.key} className="flex items-center gap-2 rounded px-2.5 py-1.5 text-[12px] text-neutral-700 hover:bg-neutral-50 transition-colors cursor-pointer">
                      <input type="checkbox" checked={presetForm.capabilities[field.key]} onChange={e => updatePresetCapability(field.key, e.target.checked)} className="h-3.5 w-3.5 rounded border-neutral-300" />
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <div>
                  <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('工作区')}</label>
                ...
                <div>
                  <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('触发')}</label>
                ...
                <div>
                  <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('会话')}</label>
                ...
                <div>
                  <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('队列')}</label>
          ...
          <div>
            <div className="flex items-center justify-between gap-3 mb-4">
              <span className="text-[12px] text-neutral-400">{t('{count} 个成员预设', { count: presets.length })}</span>
            ...
            {presetsIsError ? (
              ...
              <div className="rounded-xl border border-dashed border-neutral-200 bg-neutral-50/50 py-16 text-center">
                <p className="text-sm text-neutral-400">{t('暂无成员预设')}</p>
                <Button size="sm" className="mt-3" onClick={openCreatePreset}>
                ...
                {presets.map(preset => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-start gap-3 rounded-xl border border-neutral-200 bg-white p-4 text-left transition-all hover:border-neutral-300 hover:shadow-sm"
                    >
                      <MemberAvatar name={preset.name} avatar={preset.avatar} className="h-10 w-10 text-xs shrink-0" />
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-[13px] font-semibold text-neutral-900">{preset.name}</div>
                        {preset.aliases.length > 0 && (
                          <div className="truncate text-[11px] text-neutral-400 mt-0.5">{preset.aliases.join(', ')}</div>
                        )}
                        <div className="truncate text-[11px] text-neutral-500 mt-1">
        ...
        mobileShowEditor ? (
          ...
          <div className="rounded-xl border border-neutral-200 bg-white">
            <div className="flex items-center justify-between gap-3 border-b border-neutral-100 px-5 py-3">
              ...
              <div className="flex items-center gap-2">
                {templateMode === 'edit' && selectedTemplate && (
                  <button onClick={() => setDeleteTemplateTarget(selectedTemplate)} className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-400 hover:text-red-500 hover:bg-red-50 transition-colors" title={t('删除')}>
                    <Trash2 size={14} />
                  </button>
                )}
            ...
            <div className="p-5 space-y-5">
              <div>
                <label className="block text-[12px] font-medium text-neutral-600 mb-1">{t('模板名称')}</label>
                <input value={templateForm.name} onChange={e => updateTemplateField('name', e.target.value)} className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm transition-colors focus:border-neutral-400 focus:outline-none" />
              </div>

              <div className="grid gap-4 xl:grid-cols-2">
                <div>
                  <label className="block text-[12px] font-medium text-neutral-600 mb-2">{t('可用预设')}</label>
                  <div className="max-h-[400px] space-y-1 overflow-y-auto scrollbar-app-thin">
                    {presetsIsError ? (
                      <QueryErrorNotice title={t('加载失败')} error={presetsError} isFetching={presetsFetching} onRetry={() => void refetchPresets()} />
                    ) : presets.length === 0 ? (
                      <div className="rounded-lg border border-dashed border-neutral-200 p-3 text-center text-[12px] text-neutral-400">{t('先创建成员预设')}</div>
                    ) : (
                      presets.map(preset => {
                        const count = templateForm.memberPresetIds.filter(id => id === preset.id).length
                        return (
                          <button key={preset.id} type="button" onClick={() => addTemplatePreset(preset.id)} className={cn('flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-left transition-all', count > 0 ? 'border-neutral-900 bg-neutral-50' : 'border-neutral-200 bg-white hover:border-neutral-300')}>
                            <MemberAvatar name={preset.name} avatar={preset.avatar} className="h-7 w-7 text-[10px] shrink-0" />
                            <div className="min-w-0 flex-1">
                              <div className="truncate text-[12px] font-medium text-neutral-900">{preset.name}</div>
                            </div>
                ...
                <div>
                  <label className="block text-[12px] font-medium text-neutral-600 mb-2">{t('已选成员')}</label>
                  <div className="max-h-[400px] space-y-1 overflow-y-auto scrollbar-app-thin">
                    {selectedTemplateRows.length === 0 ? (
                      <div className="rounded-lg border border-dashed border-neutral-200 p-3 text-center text-[12px] text-neutral-400">{t('尚未选择')}</div>
                    ) : (
                      selectedTemplateRows.map((preset, index) => (
                        <div key={`${preset.id}-${index}`} className="flex items-center justify-between gap-2 rounded-lg border border-neutral-200 bg-white px-3 py-2">
          ...
          <div>
            <div className="flex items-center justify-between gap-3 mb-4">
              <span className="text-[12px] text-neutral-400">{t('{count} 个团队模板', { count: templates.length })}</span>
            ...
            {templatesIsError ? (
              ...
              <div className="rounded-xl border border-dashed border-neutral-200 bg-neutral-50/50 py-16 text-center">
                <p className="text-sm text-neutral-400">{t('暂无团队模板')}</p>
                ...
                {templates.map(template => {
                  ...
                  return (
                    <button
                      ...
                      className="flex flex-col rounded-xl border border-neutral-200 bg-white p-4 text-left transition-all hover:border-neutral-300 hover:shadow-sm"
                    >
                      <div className="truncate text-[13px] font-semibold text-neutral-900">{template.name}</div>
                      <div className="text-[11px] text-neutral-400 mt-1">{t('{count} 个成员', { count: memberCount })}</div>
                      {memberNames.length > 0 && (
                        <div className="truncate text-[11px] text-neutral-500 mt-1.5">
      ...
      <Modal isOpen={isAvatarPickerOpen} onClose={() => setIsAvatarPickerOpen(false)} title={t('选择预设头像')} className="max-w-2xl">
        <div className="grid grid-cols-4 gap-2 sm:grid-cols-5">
          {AVATAR_PRESETS.map(preset => {
            ...
            return (
              <button key={preset.id} type="button" onClick={() => handleSelectPresetAvatar(preset.src)} className={cn('group rounded-xl border bg-white p-2 text-center transition-all hover:border-neutral-400 hover:bg-neutral-50 focus:outline-none focus:ring-2 focus:ring-neutral-900/15', selected ? 'border-neutral-900 ring-2 ring-neutral-900/10' : 'border-neutral-200')} aria-pressed={selected} aria-label={t('选择 {name} 头像', { name: preset.label })}>
                <img src={preset.src} alt={preset.label} loading="lazy" className="mx-auto h-14 w-14 rounded-full object-cover sm:h-16 sm:w-16" />
                <span className="mt-1 block truncate text-[10px] font-medium text-neutral-500">{preset.label}</span>
              </button>
            )
          })}
        </div>
