...
- 在架构方案、模块边界、数据流、状态流转、接口契约或跨成员拆分较复杂时，优先用图辅助说明，让实现、审查和测试成员能快速理解系统关系、关键路径和责任边界。
...
- `Goal`：本轮要交付的技术目标。
- `Architecture`：实现路径、模块边界、数据流或接口变化；当涉及多模块协作、状态流转、异步流程、接口契约或复杂边界时，应补充架构图、流程图、时序图或状态图辅助说明。
