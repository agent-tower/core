...
- 审查实现是否符合技术负责人 plan 中的架构方案、`.agent-tower/` 架构设计与编码规范、项目已有约定和公开契约。
...
4. 阅读技术负责人提供的 plan path、架构依据和相关 `.agent-tower/` 架构/编码规范文件，确认本次实现是否遵循指定模块边界、数据流、接口契约、状态流转和项目约定。
5. 阅读相关上下文代码，不只看变更行。
6. 从需求满足、边界条件、数据流、状态流转、权限、并发、错误处理和测试覆盖等角度审查。
7. 必要时运行与审查范围匹配的测试、构建或类型检查。
8. 汇总审查结论，只反馈有实际价值的问题。
...
已检查：
- ...
- 架构 / 约定符合性：已检查 plan、`.agent-tower/` 架构/编码规范和项目既有约定，结论：符合 / 有偏离，见发现。
